...
class DocSpecObjectManager extends CommonObject
{
	...
	// contient les contexts pour lequels nous voulons prendre en charge la documetation

	public $TGlobalContextManaged = [];
	public $currentContext;
	public $suffixe = "helper\.class\.php"; 
	// If this object has a subtable with lines

	// /**
	//  * @var string    Name of subtable line
	//  */
	// public $table_element_line = 'specanddoc_docspecobjectmanagerline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_docspecobjectmanager';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'DocSpecObjectManagerline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'DocSpecObjectManager', 'fk_element'=>'fk_docspecobjectmanager'));
	...
	//  */
	// protected $childtablesoncascade = array('specanddoc_docspecobjectmanagerdet');

	// /**
	//  * @var DocSpecObjectManagerLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function getLangsContext() : string {
		global $langs;	
	 ...
	 * 
	 */
	public function fichierExisteAvecSuffixe($repertoire, $suffixe) {
		...
		foreach ($fichiers as $fichier) {
			...
			if (preg_match($regex, $fichier)) {
				return true; // Le fichier avec le suffixe existe
			}
		}
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('specanddoc', 'docspecobjectmanager', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('specanddoc', 'docspecobjectmanager_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'docspecobjectmanager/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'docspecobjectmanager/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'docspecobjectmanager/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'docspecobjectmanager/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->specanddoc->dir_output.'/docspecobjectmanager/'.$oldref;
				$dirdest = $conf->specanddoc->dir_output.'/docspecobjectmanager/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->specanddoc->dir_output.'/docspecobjectmanager/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('specanddoc','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('specanddoc','specanddoc_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SPECANDDOC_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SPECANDDOC_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SPECANDDOC_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDocSpecObjectManager")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DocSpecObjectManager").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/specanddoc/docspecobjectmanager_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDocSpecObjectManager");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DocSpecObjectManagerLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_docspecobjectmanager = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SPECANDDOC_MYOBJECT_ADDON')) {
			$conf->global->SPECANDDOC_MYOBJECT_ADDON = 'mod_docspecobjectmanager_standard';
		}

		if (getDolGlobalString('SPECANDDOC_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('SPECANDDOC_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('SPECANDDOC_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/specanddoc/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_docspecobjectmanager';
		...
		$modelpath = "core/modules/specanddoc/doc/";
...
/**
 * Class DocSpecObjectManagerLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DocSpecObjectManagerLine extends CommonObjectLine
{
	// To complete with content of an object DocSpecObjectManagerLine
	// We should have a field rowid, fk_docspecobjectmanager and position
